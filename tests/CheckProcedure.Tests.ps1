...
Describe "Check stored procedure" {
    It "Gets stored procedure exist" {
        ...
        $result = (Invoke-DbaQuery -SqlInstance $sqlServerName -Database $sqlDatabaseName -SqlCredential $credential -Query "select count(*) from sys.procedures where name = '$ProcedureName'")[0]
        $result | Should -Be 1
    }
}